...
class Pemilih extends BaseController
{
    ...
    {
        if (!CommonValidation::validateCompletionRegisterRequest($this->request, $this->validation)) {
            ...
            $resultData = $this->userService->validatePersonByID($registerModel->getParent());
            if (empty($resultData)) {
                $this->message = 'Koordinator relawan tidak ditemukan';
            } else {
                $data = $this->userService->updatePemilih($registerModel);
